...
我们将上面的代码简化来看，首先 Util 模块是直接搬运自 LazyVim 的，如果进一步学习neovim，可以直接使用，不用太过深究实现方法。

随后，其中的 load 方法也是搬运自LazyVim，这里通过*注解*的限定传入参数只能是 `"autocmds"`、`"options"`、`"keymaps"` 中的一个，实际上实现的功能就是加载这三个模块，不必太过深究，等后面对 lazy 使用次数增多，并且阅读过 LazyVim 的源码后，这部分是很容易理解的。也就是通过这个方法，加载了在 part1 中提到的自动命令、基本配置、按键映射。
